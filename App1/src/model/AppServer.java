...
public class AppServer {
	...
	private Connection connect()
	 ...
	 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb", "MySQL55", "1234");
	 }
	 catch (Exception e)
	 {e.printStackTrace();}
	 return con;
	...
	public String readAppointments()
	 ...
	 output = "<table border=\"1\"><tr><th>Appointment ID</th><th>Patiant ID</th><th>Doctor ID</th><th>Time</th><th>Update</th><th>Remove</th></tr>";
	 ...
	 while (rs.next())
	 {
	 String appid = Integer.toString(rs.getInt("itemID"));
	 String pat_id = Integer.toString(rs.getInt("itemID"));
	 String DOC_id = Integer.toString(rs.getInt("itemID"));
	...
	public String updateAppointment(int appid, int pat_id, int DOC_id, Time time, Date date)
	 {
	 String output = "";
	 try
	 {
	 ...
	 // create a prepared statement
	 String query = "UPDATE App1 SET appid=?,pat_id=?,DOC_id=?,time=?,date=? WHERE itemID=?";
	 PreparedStatement preparedStmt = con.prepareStatement(query);
	 // binding values
	 preparedStmt.setInt(1, appid);
	 preparedStmt.setInt(2, pat_id);
